...
import { IPumpSchema, pumpSchema } from "../schema/PumpSchema";
...
export default function ConfigurePump({
  open,
  onOpenChange,
  ...
  const handleOnSubmit: SubmitHandler<IPumpSchema> = (data) => {
    console.log(data);
    mutate(data, {
      onError: async (error) => {
        console.log(error);
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="w-[90%] sm:max-w-[500px] max-h-[80vh] flex flex-col">
        <DialogHeader>
          ...
          <DialogDescription>Set up Pump</DialogDescription>
        </DialogHeader>
